...
using EvelentWalker.Project;
using EvelentWalker.Rendering;
...
namespace EvelentWalker
{
    public partial class WorldForm : Form, DXForm
    {
        ...
        int MouseX;
        int MouseY;
        System.Drawing.Point MouseDownPoint;        System.Drawing.Point MouseLastPoint;

        // ---- rubber-band (box) entity selection ----
        bool MarqueeSelecting = false;
        System.Drawing.Point MarqueeStartClient;
        System.Windows.Forms.Panel MarqueeBox;

        bool rendermaps = false;
        bool renderworld = false;
        bool showWorldGrid = Settings.Default.ShowWorldGrid;
        bool renderGtaMap = Settings.Default.RenderGtaMap;
        bool enableSpawnTestEntity = Settings.Default.EnableSpawnTestEntity;
        ...
        public WorldForm()
        {
            ...
            ToolsMenuShowGrid.Checked = showWorldGrid;

            //keep only the Tools button in the tools panel header (hide About and the show/hide arrows)
            AboutButton.Visible = false;
            ToolsPanelHideButton.Visible = false;
            ToolsPanelShowButton.Visible = false;
            ToolsPanelExpandButton.Visible = false;
            //show the tools panel by default (since the show/hide arrows are gone)
            ToolsPanel.Visible = true;
            //make the Tools button compact so it doesn't take much space
            ToolsButton.Text = "☰";
            ToolsButton.Size = new System.Drawing.Size(28, 23);

            //turn the toolbar into a vertical icon strip pinned to the bottom-left
            ConfigureVerticalToolbar();

            //round the right tools panel and move its ☰ button to the bottom-left
            ConfigureToolsPanel();

            //make the bottom status bar background invisible (white text only)
            ConfigureStatusBar();

            //add a "Keyboard Shortcuts" entry to the tools menu
            try
            {
                ToolsMenu.Items.Add(new System.Windows.Forms.ToolStripSeparator());
                var scItem = new System.Windows.Forms.ToolStripMenuItem("Keyboard Shortcuts");
                scItem.ShortcutKeyDisplayString = "F1";
                scItem.Click += (s, e) => ShowShortcuts();
                ToolsMenu.Items.Add(scItem);
            }
            catch { }

            AddRenderGtaMapOption();
            AddEnableSpawnTestEntityOption();
        ...
        /// <summary>
        /// Turns the floating toolbar into a vertical icon strip ("||||") docked to
        /// the bottom-left of the world view, instead of a horizontal bar ("----")
        /// at the top-left. Items overflow into a chevron if the window is short.
        /// </summary>
        private const int VToolbarWidth = 48;

        private void ConfigureVerticalToolbar()
        {
            try
            {
                Toolbar.LayoutStyle = System.Windows.Forms.ToolStripLayoutStyle.VerticalStackWithOverflow;
                ...
                Toolbar.Padding = new System.Windows.Forms.Padding(0, 4, 0, 4);
                Toolbar.BackColor = EvelentTheme.StripBack;

                // Give every item the same width with a centred icon so they line up
                // vertically instead of some sticking out further than others.
                int itemW = VToolbarWidth - 8;
                foreach (System.Windows.Forms.ToolStripItem item in Toolbar.Items)
                {
                    if (item is System.Windows.Forms.ToolStripSeparator) continue;
                    item.AutoSize = false;
                    item.Size = new System.Drawing.Size(itemW, 30);
                    item.ImageAlign = System.Drawing.ContentAlignment.MiddleCenter;
                    item.Margin = new System.Windows.Forms.Padding(4, 1, 4, 1);
                }

                ToolbarPanel.AutoSize = false;
                ToolbarPanel.BorderStyle = System.Windows.Forms.BorderStyle.None;
                ToolbarPanel.Anchor = System.Windows.Forms.AnchorStyles.Left | System.Windows.Forms.AnchorStyles.Top | System.Windows.Forms.AnchorStyles.Bottom;
                ToolbarPanel.BackColor = EvelentTheme.StripBack;

                LayoutVerticalToolbar();
                Resize += (s, e) => { LayoutVerticalToolbar(); LayoutToolsPanel(); };
            }
            catch { }
        }

        private void LayoutVerticalToolbar()
        {
            if (ToolbarPanel == null) return;
            const int margin = 12;
            const int toolsBtnH = 38;
            const int gap = 8;

            // Match the right Tools panel's vertical extent so both side rails line up.
            int top, height;
            if (ToolsPanel != null)
            {
                top = ToolsPanel.Top;
            ...
            {
                int statusH = (StatusStrip != null && StatusStrip.Visible) ? StatusStrip.Height : 0;
                top = margin;
                height = ClientSize.Height - statusH - margin * 2;
            }
            if (height < 120) height = 120;

            // Reserve the bottom of the rail for the ☰ tools button.
            int railH = height - toolsBtnH - gap;
            if (railH < 60) railH = 60;

            ToolbarPanel.Left = margin;
            ToolbarPanel.Top = top;
            ToolbarPanel.Size = new System.Drawing.Size(VToolbarWidth, railH);
            RoundPanel(ToolbarPanel, 16);
            ToolbarPanel.BringToFront();

            // Place the ☰ button at the very bottom of the left rail.
            if (ToolsButton != null && ToolsButton.Parent == this)
            {
                ToolsButton.SetBounds(margin, top + railH + gap, VToolbarWidth, toolsBtnH);
                RoundPanel(ToolsButton, 14);
                ToolsButton.BringToFront();
            }
        }
        ...
        public void ShowShortcuts()
        {
            try
            {
                if (shortcutsForm == null || shortcutsForm.IsDisposed)
                {
                    shortcutsForm = new ShortcutsForm(Input?.keyBindings);
                    shortcutsForm.FormClosed += (s, e) => shortcutsForm = null;
                }
                if (!shortcutsForm.Visible) shortcutsForm.Show(this);
                shortcutsForm.Focus();
            }
            catch { }
        }
        ...
        /// <summary>
        /// Rounds the right Tools panel. The ☰ button is moved out to the left
        /// rail (see ConfigureVerticalToolbar), so the tab area fills the panel.
        /// </summary>
        private void ConfigureToolsPanel()
        {
            try
            {
                // hand the ☰ button over to the form so it can live on the left rail
                if (ToolsButton.Parent != this)
                {
                    Controls.Add(ToolsButton);
                    ToolsButton.BringToFront();
                }
                ToolsButton.Anchor = System.Windows.Forms.AnchorStyles.Bottom | System.Windows.Forms.AnchorStyles.Left;
                ToolsButton.Text = "☰";
                ToolsButton.FlatStyle = System.Windows.Forms.FlatStyle.Flat;
                ToolsButton.FlatAppearance.BorderSize = 0;
                ToolsButton.BackColor = EvelentTheme.StripBack;
                ToolsButton.ForeColor = System.Drawing.Color.White;
                ToolsButton.Font = new System.Drawing.Font("Segoe UI", 12f, System.Drawing.FontStyle.Bold);

                LayoutToolsPanel();
                ToolsPanel.SizeChanged += (s, e) => LayoutToolsPanel();
                ToolsPanel.VisibleChanged += (s, e) => LayoutToolsPanel();

                // now that the ☰ button is on the form, position it on the left rail
                LayoutVerticalToolbar();
            }
            catch { }
        }
        ...
        private void LayoutToolsPanel()
        {
            ...
            int cw = ToolsPanel.ClientSize.Width;
            int ch = ToolsPanel.ClientSize.Height;

            // Fill the panel with the tab control, leaving a small uniform margin
            // (the ☰ button now lives on the left rail, so no bottom strip needed).
            if (ToolsTabControl != null)
                ToolsTabControl.SetBounds(6, 6, cw - 12, ch - 12);

            RoundPanel(ToolsPanel, 14);
        }

        private static void RoundPanel(System.Windows.Forms.Control c, int radius)
        {
            try
            {
                ...
                using (var path = new System.Drawing.Drawing2D.GraphicsPath())
                {
                    ...
                    path.CloseFigure();
                    c.Region = new System.Drawing.Region(path);
                }
            }
            catch { }
        }
        ...
        /// <summary>
        /// Gives the bottom status bar a solid dark background that matches the
        /// rest of the themed chrome, with readable light status text.
        /// </summary>
        private void ConfigureStatusBar()
        {
            try
            {
                StatusStrip.Renderer = new EvelentToolStripRenderer();
                StatusStrip.BackColor = EvelentTheme.StripBack;
                StatusStrip.ForeColor = EvelentTheme.TextColor;
                StatusStrip.SizingGrip = false;
                StatusStrip.AutoSize = true;
                var font = new System.Drawing.Font("Segoe UI", 8f);
                foreach (System.Windows.Forms.ToolStripItem item in StatusStrip.Items)
                {
                    item.BackColor = EvelentTheme.StripBack;
        ...
        public bool ConfirmQuit()
        {
            ...
            {
                if (MessageBox.Show("Are you sure you want to quit EvelentWalker?", "Confirm quit", MessageBoxButtons.YesNo) != DialogResult.Yes)
                {
                    return false;
                }
            }
        ...
        private void RenderWorld()
        {
            ...
            if ((renderwaterquads && renderGtaMap) || (SelectionMode == MapSelectionMode.WaterQuad))
            {
        ...
        private void AddRenderGtaMapOption()
        {
            //adds a "Render GTA V map" toggle to the Options > Render tab (placed below existing controls)
            try
            {
                var chk = new System.Windows.Forms.CheckBox();
                chk.Text = "Render GTA V map";
                chk.AutoSize = true;
                chk.Checked = renderGtaMap;
                chk.ForeColor = EvelentTheme.TextColor;
                int maxBottom = 6;
                foreach (System.Windows.Forms.Control c in OptionsRenderTabPage.Controls)
                {
                    if (c.Bottom > maxBottom) maxBottom = c.Bottom;
                }
                chk.Location = new System.Drawing.Point(6, maxBottom + 10);
                chk.CheckedChanged += (s, e) =>
                {
                    renderGtaMap = chk.Checked;
                    Settings.Default.RenderGtaMap = chk.Checked;
                    Settings.Default.Save();
                };
                OptionsRenderTabPage.AutoScroll = true;
                OptionsRenderTabPage.Controls.Add(chk);
            }
            catch { }
        }

        private void AddEnableSpawnTestEntityOption()
        {
            //adds an "Enable Spawn Test Entity" toggle to the Options > Render tab.
            //When enabled, Ctrl + Left mouse button spawns a test entity at the camera.
            try
            {
                var chk = new System.Windows.Forms.CheckBox();
                chk.Text = "Enable Spawn Test Entity (Ctrl+LMB)";
                chk.AutoSize = true;
                chk.Checked = enableSpawnTestEntity;
                chk.ForeColor = EvelentTheme.TextColor;
                int maxBottom = 6;
                foreach (System.Windows.Forms.Control c in OptionsRenderTabPage.Controls)
                {
                ...
                chk.CheckedChanged += (s, e) =>
                {
                    enableSpawnTestEntity = chk.Checked;
                    Settings.Default.EnableSpawnTestEntity = chk.Checked;
                    Settings.Default.Save();
                };
                OptionsRenderTabPage.AutoScroll = true;
                OptionsRenderTabPage.Controls.Add(chk);
        ...
        {
            //immediately render a reference ground grid on the Z=0 plane, centred near the camera.
            ...
            const int half = 50;
            const float extent = spacing * half;
            const float z = 0.0f;

            uint col = (uint)new Color4(0.6f, 0.6f, 0.6f, 0.35f).ToRgba();
            uint colAxisX = (uint)new Color4(0.85f, 0.3f, 0.3f, 0.7f).ToRgba();
            uint colAxisY = (uint)new Color4(0.3f, 0.75f, 0.3f, 0.7f).ToRgba();

            var cp = camera.Position;
            float cx = (float)Math.Round(cp.X / spacing) * spacing;
            float cy = (float)Math.Round(cp.Y / spacing) * spacing;

            for (int i = -half; i <= half; i++)
            {
                float x = cx + i * spacing;
                float y = cy + i * spacing;
                //lines parallel to Y axis (constant X); centre line tinted green (Y axis)
                Renderer.RenderSelectionLine(new Vector3(x, cy - extent, z), new Vector3(x, cy + extent, z), (i == 0) ? colAxisY : col);
                //lines parallel to X axis (constant Y); centre line tinted red (X axis)
                Renderer.RenderSelectionLine(new Vector3(cx - extent, y, z), new Vector3(cx + extent, y, z), (i == 0) ? colAxisX : col);
            }
        }
        ...
        private void UpdateToolbarShortcutsText()
        {
            ...
            ToolbarSelectButton.ToolTipText = string.Format("Select objects / Exit edit mode ({0}, {1})\r\nIn Entity mode: hold Shift and drag to box-select (Ctrl = add)", kb.ToggleMouseSelect, kb.ExitEditMode);
        ...
        private void ShowCutsceneForm()
        {
            if (CutsceneForm == null)
            {
                SaveCameraStateForCutscene(); //remember the camera so we can restore it when the viewer closes
        ...
        {
            CutsceneForm = null;
            RestoreCameraStateAfterCutscene(); //put the camera back where it was before the cutscene
            //ToolbarCutsceneWindowButton.Checked = false;
        }

        // ---- save/restore the free camera around cutscene playback ----
        private bool cutsceneCamSaved = false;
        private Vector3 savedCamFollowPos;
        private Quaternion savedCamFollowOri;
        private Vector3 savedCamTargetRot;
        private float savedCamTargetDist;
        private float savedCamZNear;
        private float savedCamZFar;

        private void SaveCameraStateForCutscene()
        {
            if (cutsceneCamSaved) return; //keep the original state if the viewer is reopened
            if ((camera == null) || (camera.FollowEntity == null)) return;
            savedCamFollowPos = camera.FollowEntity.Position;
            savedCamFollowOri = camera.FollowEntity.Orientation;
            savedCamTargetRot = camera.TargetRotation;
            savedCamTargetDist = camera.TargetDistance;
            savedCamZNear = camera.ZNear;
            savedCamZFar = camera.ZFar;
            cutsceneCamSaved = true;
        }

        private void RestoreCameraStateAfterCutscene()
        {
            if (!cutsceneCamSaved) return;
            cutsceneCamSaved = false;
            if ((camera == null) || (camera.FollowEntity == null)) return;
            lock (Renderer.RenderSyncRoot)
            {
                camera.FollowEntity.Position = savedCamFollowPos;
                camera.FollowEntity.Orientation = savedCamFollowOri;
                camera.FollowEntity.OrientationInv = Quaternion.Invert(savedCamFollowOri);
                camera.TargetRotation = savedCamTargetRot;
                camera.CurrentRotation = savedCamTargetRot;
                camera.TargetDistance = savedCamTargetDist;
                camera.CurrentDistance = savedCamTargetDist;
                camera.ZNear = savedCamZNear;
                camera.ZFar = savedCamZFar;
                camera.UpdateProj = true;
            }
        }
        ...
        private void ContentThread()
        {
            ...
            {
                MessageBox.Show("Keys not found! This shouldn't happen, GTA5.exe outdated? EvelentWalker outdated?");
                Close();
                return;
            }
        ...
        private void ResetSettings()
        {
            ...
            MessageBox.Show("All settings have been reset to their default values. Please restart EvelentWalker.");
        }
        ...
        //pushes an externally-created undo step (e.g. entity add/delete from the project window)
        public void RegisterUndoStep(UndoStep s)
        {
            if (s == null) return;
            RedoSteps.Clear();
            UndoSteps.Push(s);
            if (InvokeRequired) { try { BeginInvoke(new Action(UpdateUndoUI)); } catch { } }
            else UpdateUndoUI();
        }

        //public undo/redo entry points (used by the Project window toolbar/menu)
        public bool CanUndo { get { return UndoSteps.Count > 0; } }
        public bool CanRedo { get { return RedoSteps.Count > 0; } }
        public void PerformUndo()
        {
            if (InvokeRequired) { try { Invoke(new Action(PerformUndo)); } catch { } return; }
            Undo();
        }
        public void PerformRedo()
        {
            if (InvokeRequired) { try { Invoke(new Action(PerformRedo)); } catch { } return; }
            Redo();
        }

        //called by EntityGraphUndoStep after restoring a ymap's entities, to refresh the tree + 3D view
        public void OnEntityGraphUndoRedo(YmapFile ymap)
        {
            try { SelectItem(null); } catch { }
            if (ProjectForm != null)
            {
                try { ProjectForm.OnEntityGraphChanged(ymap); } catch { }
            }
        }
        ...
        private void UpdateUndoUI()
        {
            ...
            //keep the Project window's Undo/Redo buttons + menu in sync
            try { ProjectForm?.UpdateUndoRedoButtons(UndoSteps.Count > 0, RedoSteps.Count > 0); } catch { }
        }
        ...
        private void DeleteEntity(YmapEntityDef ent)
        {
            ...
            var beforeState = ymap?.CaptureEntityState();
            if (ymap == null)
            {
                if (instance != null)
                {
            ...
            {
                RegisterUndoStep(new EntityGraphUndoStep(ymap, beforeState, ymap.CaptureEntityState(), "Delete entity"));
                SelectItem(null);
            }
        }
        ...
        {
            //hide the window immediately so closing feels instant; heavy teardown happens afterwards
            if (!e.Cancel)
            {
                try { Hide(); } catch { }
            }
        ...
        private void WorldForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            //if this is a standalone launch (no menu window behind us), terminate the process
            //immediately instead of waiting for the slow managed teardown of render threads,
            //the game file cache, DirectX resources, etc.
            bool menuOpen = Application.OpenForms.OfType<MenuForm>().Any();
            if (!menuOpen)
            {
                Environment.Exit(0);
            }
        ...
        private void WorldForm_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            //Ctrl + left mouse button spawns a test entity at the camera (when enabled in Options)
            if (enableSpawnTestEntity && (e.Button == MouseButtons.Left) && ((ModifierKeys & Keys.Control) != 0)
                && (ControlMode == WorldControlMode.Free) && !ControlBrushEnabled)
            {
                SpawnTestEntity(true);
                MouseX = e.X;
                MouseY = e.Y;
            ...
            //Shift + left-drag in Entity select mode starts a rubber-band box selection
            if (TryBeginMarqueeSelection(e))
            {
                MouseX = e.X;
                MouseY = e.Y;
                return;
        ...
        private void WorldForm_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if ((e.Button == MouseButtons.Left) && MarqueeSelecting)
            {
                FinishMarqueeSelection(e, (ModifierKeys & Keys.Control) > 0);
                return;
            }
        ...
        private void WorldForm_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (MarqueeSelecting)
            {
                UpdateMarqueeFrame(e);
                UpdateMousePosition(e);
                return; //don't rotate the camera while box-selecting
            }
            ...
            if (ControlMode == WorldControlMode.Free && !ControlBrushEnabled)
            {
                ...
                if (MouseRButtonDown)
                {
                    ...
                    //time-of-day is no longer changed by right-mouse drag - use the Time of day slider instead
                }
        ...
        // ---------------------------------------------------------------------
        //  Rubber-band (box) entity selection - Shift + left-drag
        // ---------------------------------------------------------------------
        private bool TryBeginMarqueeSelection(MouseEventArgs e)
        {
            if (e.Button != MouseButtons.Left) return false;
            if (!MouseSelectEnabled) return false;
            if (SelectionMode != MapSelectionMode.Entity) return false;
            if ((ModifierKeys & Keys.Shift) == 0) return false;
            if (ControlMode != WorldControlMode.Free) return false;
            if (ControlBrushEnabled) return false;
            if (MousedMarker != null) return false;
            if (ShowWidget && Widget.IsUnderMouse) return false; //don't hijack widget cloning/dragging

            MarqueeSelecting = true;
            MarqueeStartClient = e.Location;
            try { Cursor = Cursors.Cross; } catch { }
            return true;
        }

        private void EnsureMarqueeBox()
        {
            if (MarqueeBox != null) return;
            MarqueeBox = new System.Windows.Forms.Panel
            {
                BackColor = System.Drawing.Color.FromArgb(80, 160, 255), //thin bright outline (only the border ring is shown via Region)
                Visible = false,
                Enabled = false //click-through; it must never steal mouse input
            };
            Controls.Add(MarqueeBox);
        }

        private void UpdateMarqueeFrame(MouseEventArgs e)
        {
            EnsureMarqueeBox();

            //rectangle in the form's client coordinates - exactly where the cursor is
            var r = System.Drawing.Rectangle.FromLTRB(
                Math.Min(MarqueeStartClient.X, e.X), Math.Min(MarqueeStartClient.Y, e.Y),
                Math.Max(MarqueeStartClient.X, e.X), Math.Max(MarqueeStartClient.Y, e.Y));
            if (r.Width < 1) r.Width = 1;
            if (r.Height < 1) r.Height = 1;

            MarqueeBox.Bounds = r;

            //carve out the middle so only a 2px outline is drawn and the scene shows through
            const int t = 2;
            var outer = new System.Drawing.Rectangle(0, 0, r.Width, r.Height);
            var region = new System.Drawing.Region(outer);
            var inner = System.Drawing.Rectangle.Inflate(outer, -t, -t);
            if ((inner.Width > 0) && (inner.Height > 0)) region.Exclude(inner);
            var oldRegion = MarqueeBox.Region;
            MarqueeBox.Region = region;
            oldRegion?.Dispose();

            if (!MarqueeBox.Visible) MarqueeBox.Visible = true;
            MarqueeBox.BringToFront();
        }

        private void HideMarqueeBox()
        {
            if (MarqueeBox != null) MarqueeBox.Visible = false;
        }

        private void FinishMarqueeSelection(MouseEventArgs e, bool additive)
        {
            ...
            var rect = System.Drawing.Rectangle.FromLTRB(
                Math.Min(MarqueeStartClient.X, e.Location.X), Math.Min(MarqueeStartClient.Y, e.Location.Y),
                Math.Max(MarqueeStartClient.X, e.Location.X), Math.Max(MarqueeStartClient.Y, e.Location.Y));

            //a tiny rectangle is really just a click - leave the current selection alone
            if ((rect.Width < 3) && (rect.Height < 3)) return;

            float w = camera.Width;
            float h = camera.Height;

            var found = new List<YmapEntityDef>();
            var seen = new HashSet<YmapEntityDef>();

            lock (Renderer.RenderSyncRoot) //RenderedDrawables is rebuilt on the render thread
            {
                foreach (var rd in Renderer.RenderedDrawables)
                {
                    var ent = rd.Entity;
                    ...
                    var sp = camera.ViewProjMatrix.MultiplyW(ent.Position - camera.Position);
                    if (sp.Z <= 0.0f) continue; //behind the camera

                    float px = ((sp.X * 0.5f) + 0.5f) * w;
                    float py = ((sp.Y * -0.5f) + 0.5f) * h;
            ...
            if (found.Count == 0)
            {
                if (!additive) SelectItem(null); //empty box clears the selection
                return;
            }

            var items = new MapSelection[found.Count];
            for (int i = 0; i < found.Count; i++)
            {
                items[i] = MapSelection.FromProjectObject(this, found[i]);
            }

            SelectMulti(items, additive, true);
        }
        ...
        private void ToolsMenuConfigureGame_Click(object sender, EventArgs e)
        {
            ...
            if (result)
            {
                MessageBox.Show("Please restart EvelentWalker to use the new folder.");
            }
        }
        ...
        private void ToolsMenuShowGrid_Click(object sender, EventArgs e)
        {
            showWorldGrid = ToolsMenuShowGrid.Checked;
            Settings.Default.ShowWorldGrid = showWorldGrid;
            Settings.Default.Save();
        }
