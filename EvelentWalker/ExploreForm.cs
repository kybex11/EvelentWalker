    ...
    public partial class ExploreForm : Form
    {
        ...
        private void DefragmentSelected()
        {
            ...
            Form form = new Form() {
                ...
                Text = "Defragment RPF Archive - EvelentWalker",
        ...
        {
            return Path.Combine(Path.GetTempPath(), "EvelentWalkerDrop");
        }
        ...
        private void ExploreForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            //with a large amount of RPF data cached, the managed teardown (gen2 GC of many GB)
            //can freeze the process for a long time on close. If RPF Explorer was launched
            //standalone, just terminate immediately and let the OS reclaim memory instantly.
            bool primaryOpen = false;
            foreach (Form f in Application.OpenForms)
            {
                if (f is WorldForm || f is MenuForm) { primaryOpen = true; break; }
            }
            if (!primaryOpen)
            {
                Environment.Exit(0);
