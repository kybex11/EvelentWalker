...
{
    /// <summary>
    /// Generic undo/redo step driven by two delegates. Used to make almost any
    /// create/delete/add/remove operation reversible: the operation supplies an
    /// action that re-applies it (redo) and one that reverses it (undo).
    /// </summary>
    public class RelayUndoStep : UndoStep
    {
        private readonly Action UndoAction;
        private readonly Action RedoAction;
        private readonly string Description;

        public RelayUndoStep(string description, Action undoAction, Action redoAction)
        {
    ...
    /// <summary>
    /// Undo/redo for ymap entity graph changes (creating or deleting entities,
    /// including whole LOD groups). Stores a full before/after snapshot of the
    /// ymap's entity arrays and swaps between them.
    /// </summary>
    public class EntityGraphUndoStep : UndoStep
    {
        private readonly YmapFile Ymap;
        private readonly YmapEntityState Before;
        private readonly YmapEntityState After;
        private readonly string Description;

        public EntityGraphUndoStep(YmapFile ymap, YmapEntityState before, YmapEntityState after, string description)
        {
            Ymap = ymap;
            Before = before;
            After = after;
        ...
        public override void Undo(WorldForm wf, ref MapSelection sel)
        {
            Apply(wf, ref sel, Before);
        }

        public override void Redo(WorldForm wf, ref MapSelection sel)
        {
            Apply(wf, ref sel, After);
        }

        private void Apply(WorldForm wf, ref MapSelection sel, YmapEntityState state)
        {
            if ((Ymap == null) || (state == null)) return;

            if (wf != null)
            {
                lock (wf.RenderSyncRoot)
                {
                    Ymap.RestoreEntityState(state);
                }
            }
            else
            {
            ...
            sel = new MapSelection(); //the previously selected entity may no longer exist
            wf?.OnEntityGraphUndoRedo(Ymap);
        }

        public override string ToString()
        {
            return Description ?? "Entity change";
        }
    }
