...
using EvelentWalker.Project.Panels;
using EvelentWalker.Properties;
using EvelentWalker.Utils;
...
namespace EvelentWalker.Project
{
    public partial class ProjectForm : Form
    {
        ...
        public ProjectForm(WorldForm worldForm = null)
        {
            ...
            //ensure every panel docked into the project window gets the dark theme applied
            MainDockPanel.ContentAdded += MainDockPanel_ContentAdded;

            SetTheme(Settings.Default.ProjectWindowTheme, false);
            ShowDefaultPanels();

            if (EvelentTheme.AppIcon != null) { try { Icon = EvelentTheme.AppIcon; } catch { } }
        ...
        private void MainDockPanel_ContentAdded(object sender, DockContentEventArgs e)
        {
            //apply the EvelentWalker dark theme to any panel added to the dock (editors, explorer, etc.)
            if (e.Content is Control c)
            {
                try
                {
                    c.BackColor = EvelentTheme.WindowBack;
                    c.ForeColor = EvelentTheme.TextColor;
                    EvelentTheme.ApplyToControls(c);
                }
                catch { }
            }
        }
        ...
        public void NewProject()
        {
            ...
            CurrentProjectFile.Name = "New EvelentWalker Project";
        ...
        {
            string file = ShowOpenDialog("EvelentWalker Projects|*.evproj", string.Empty);
        ...
        public void CloseProject()
        {
            ...
            //request a non-blocking background collection so the (potentially multi-GB) project
            //data is reclaimed without freezing the UI
            try { GC.Collect(GC.MaxGeneration, GCCollectionMode.Optimized, false); } catch { }
        }
        public void SaveProject(bool saveas = false)
        {
            ...
            {
                string fileName = ShowSaveDialog("EvelentWalker Projects|*.evproj", CurrentProjectFile.Filepath);
        ...
        public void OpenFiles(string[] files = null)
        {
            ...
            foreach (var file in files)
            {
                ...
                //keep the UI message pump alive during loading so the world view stays
                //responsive (camera/flying) instead of freezing while many files load
                Application.DoEvents();
            }
        ...
        // ----- Project tree right-click context actions -----
        public bool TreeItemCanDelete(object tag)
        {
            switch (tag)
            {
                case YmapEntityDef _:
                case MCEntityDef _:
                case YmapCarGen _:
                case YmapLODLight _:
                case YmapBoxOccluder _:
                case YmapOccludeModelTriangle _:
                case YmapGrassInstanceBatch _:
                case Bounds _:
                case BoundPolygon _:
                case BoundVertex _:
                case YndNode _:
                ...
                case ScenarioNode _:
                case MCMloRoomDef _:
                case MCMloPortalDef _:
                case MCMloEntitySet _:
                case Archetype _:
                    return true;
            }
            return false;
        }
        public void TreeItemDelete(object tag)
        {
            switch (tag)
            {
                case YmapEntityDef _:
                case MCEntityDef _: DeleteEntity(); break;
                case YmapCarGen _: DeleteCarGen(); break;
                case YmapLODLight _: DeleteLodLight(); break;
                case YmapBoxOccluder _: DeleteBoxOccluder(); break;
                case YmapOccludeModelTriangle _: DeleteOccludeModelTriangle(); break;
                case YmapGrassInstanceBatch _: DeleteGrassBatch(); break;
                case BoundVertex _: DeleteCollisionVertex(); break;
                case BoundPolygon _: DeleteCollisionPoly(); break;
                case Bounds _: DeleteCollisionBounds(); break;
                case YndNode _: DeletePathNode(); break;
                case TrainTrackNode _: DeleteTrainNode(); break;
                case ScenarioNode _: DeleteScenarioNode(); break;
                case MCMloRoomDef _: DeleteMloRoom(); break;
                case MCMloPortalDef _: DeleteMloPortal(); break;
                case MCMloEntitySet _: DeleteMloEntitySet(); break;
                case Archetype _: DeleteArchetype(); break;
            }
        }
        public bool TreeItemCanRemoveFromProject(object tag)
        {
            switch (tag)
            {
                case YmapFile _:
        ...
        public void TreeItemRemoveFromProject(object tag)
        {
            switch (tag)
            {
                case YmapFile _: RemoveYmapFromProject(); break;
                case YtypFile _: RemoveYtypFromProject(); break;
                case YbnFile _: RemoveYbnFromProject(); break;
                case YndFile _: RemoveYndFromProject(); break;
                case YnvFile _: RemoveYnvFromProject(); break;
                case YdrFile _: RemoveYdrFromProject(); break;
                case YddFile _: RemoveYddFromProject(); break;
                case YftFile _: RemoveYftFromProject(); break;
                case YtdFile _: RemoveYtdFromProject(); break;
            }
        }

        /// <summary>
        /// Whether the given tree item supports in-place renaming.
        /// </summary>
        public bool TreeItemCanRename(object tag)
        {
            return tag is Archetype;
        }
        public void TreeItemRename(object tag, string newName)
        {
            if (string.IsNullOrWhiteSpace(newName)) return;
            if (tag is Archetype arch)
            {
                uint hash;
                if (!uint.TryParse(newName, out hash)) hash = JenkHash.GenHash(newName);
                if (arch._BaseArchetypeDef.name != hash)
                {
        ...
        /// <summary>
        /// Returns the "New ..." creation actions appropriate for the given tree node tag.
        /// Used to build the right-click context menu inside the project tree.
        /// </summary>
        public List<KeyValuePair<string, Action>> GetTreeNewActions(object tag)
        {
            var list = new List<KeyValuePair<string, Action>>();
            void add(string n, Action a) => list.Add(new KeyValuePair<string, Action>(n, a));

            if (tag is YmapFile)
            {
                add("Entity", () => NewEntity());
                add("Car Generator", () => NewCarGen());
                add("Grass Batch", () => NewGrassBatch());
            }
            else if (tag is YtypFile)
            {
                add("Archetype", () => NewArchetype());
            }
            else if (tag is YbnFile)
            {
                add("Bounds (Box)", () => NewCollisionBounds(BoundsType.Box));
                add("Bounds (Sphere)", () => NewCollisionBounds(BoundsType.Sphere));
                add("Bounds (Geometry)", () => NewCollisionBounds(BoundsType.Geometry));
                add("Bounds (Composite)", () => NewCollisionBounds(BoundsType.Composite));
            }
            else if (tag is YndFile)
            {
                add("Path Node", () => NewPathNode());
            }
            else if (tag is YnvFile)
            {
                add("Polygon", () => NewNavPoly());
            }
            else if (tag is TrainTrack)
            {
                add("Train Node", () => NewTrainNode());
            }
            else
            {
                //project root, category folders or empty area -> offer new top-level files
                add("Ymap", () => NewYmap());
                add("Ytyp", () => NewYtyp());
                add("Ybn", () => NewYbn());
                add("Ynd", () => NewYnd());
                add("Ynv", () => NewYnv());
                add("Train Track", () => NewTrainTrack());
                add("Scenario", () => NewScenario());
                add("Audio", () => NewAudioFile());
            }
            return list;
        }
        ...
        public YmapEntityDef NewEntity(YmapEntityDef copy = null, bool copyPosition = false, bool selectNew = true)
        {
            ...
            var undoBefore = CurrentYmapFile.CaptureEntityState();
            if (WorldForm != null)
            {
            ...
            WorldForm?.RegisterUndoStep(new EntityGraphUndoStep(CurrentYmapFile, undoBefore, CurrentYmapFile.CaptureEntityState(), "Create entity"));
        ...
        private bool DeleteYmapEntity()
        {
            ...
            //if the entity takes part in a LOD hierarchy (it has a parent and/or
            //children), deleting it on its own would break the LOD links and can
            //crash the game. Offer to delete the whole LOD group instead.
            if (EntityHasLodLinks(CurrentEntity))
            {
                return DeleteYmapEntityLodGroup();
            }
            ...
            {
                WorldForm?.RegisterUndoStep(new EntityGraphUndoStep(CurrentYmapFile, undoBefore, CurrentYmapFile.CaptureEntityState(), "Delete entity"));
            }
        ...
        /// <summary>
        /// True when the entity participates in a LOD hierarchy - it has children,
        /// or a parent that lives in the same ymap.
        /// </summary>
        private bool EntityHasLodLinks(YmapEntityDef ent)
        {
            if (ent == null) return false;
            if (ent._CEntityDef.numChildren != 0) return true;
            if ((ent.Children?.Length ?? 0) != 0) return true;
            if ((ent.Parent != null) && (ent.Parent.Ymap == ent.Ymap)) return true;
            return false;
        }

        /// <summary>
        /// Collects the complete LOD group an entity belongs to: the top-most
        /// in-ymap ancestor plus every descendant. The group is self-contained,
        /// so removing it as a whole keeps the ymap's LOD links consistent.
        /// </summary>
        private List<YmapEntityDef> GetEntityLodGroup(YmapEntityDef ent)
        {
            var result = new List<YmapEntityDef>();
            if (ent == null) return result;

            //walk up to the top-most ancestor that still lives in this ymap
            var root = ent;
            int guard = 0;
            while ((root.Parent != null) && (root.Parent.Ymap == ent.Ymap) && !root.LodInParentYmap && (guard++ < 4096))
            {
                root = root.Parent;
            }

            //collect root + all descendants (within this ymap)
            var seen = new HashSet<YmapEntityDef>();
            var stack = new Stack<YmapEntityDef>();
            stack.Push(root);
            while (stack.Count > 0)
            {
                var e = stack.Pop();
                ...
                if (e.Children != null)
                {
                    ...
                    {
                        if ((c != null) && (c.Ymap == ent.Ymap)) stack.Push(c);
                    }
                }
            }
            return result;
        ...
        private bool DeleteYmapEntityLodGroup()
        {
            var group = GetEntityLodGroup(CurrentEntity);
            if (group.Count == 0) return true;

            var root = group[0];

            var msg = "'" + CurrentEntity.Name + "' is part of a LOD hierarchy (it has a parent and/or child entities)." + Environment.NewLine + Environment.NewLine
                    + "Deleting only this entity would break the LOD links and can corrupt the game." + Environment.NewLine + Environment.NewLine
                    + "Delete the ENTIRE LOD group instead?" + Environment.NewLine + Environment.NewLine
                    + "    Top parent:  " + root.Name + Environment.NewLine
                    + "    Entities:    " + group.Count + Environment.NewLine + Environment.NewLine
                    + "This cannot be undone.";
            if (MessageBox.Show(msg, "Delete LOD group", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes)
            {
                return true; //user declined - leave everything untouched
            }

            var delymap = CurrentYmapFile;
            var undoBefore = delymap.CaptureEntityState();
            if (WorldForm != null)
            {
                lock (WorldForm.RenderSyncRoot) //don't try to do this while rendering...
                {
                    delymap.RemoveEntities(group);
                }
            }
            else
            {
            ...
            WorldForm?.RegisterUndoStep(new EntityGraphUndoStep(delymap, undoBefore, delymap.CaptureEntityState(), "Delete LOD group (" + group.Count + ")"));

            foreach (var e in group)
            {
                ProjectExplorer?.RemoveEntityTreeNode(e);
                ClosePanel((EditYmapEntityPanel p) => { return p.Tag == e; });
            }
            ProjectExplorer?.SetYmapHasChanged(delymap, true);
        ...
        public YmapGrassInstanceBatch NewGrassBatch(YmapGrassInstanceBatch copy = null)
        {
            ...
            {
                var uymap = CurrentYmapFile; var ubatch = batch;
                RegisterUndoItem("Create grass batch",
                    () => RunLocked(() => uymap.AddGrassBatch(ubatch)),
                    () => RunLocked(() => uymap.RemoveGrassBatch(ubatch)));
            }
        ...
        public bool DeleteGrassBatch()
        {
            ...
            {
                var uymap = CurrentYmapFile; var ubatch = delbatch;
                RegisterUndoItem("Delete grass batch",
                    () => RunLocked(() => uymap.RemoveGrassBatch(ubatch)),
                    () => RunLocked(() => uymap.AddGrassBatch(ubatch)));
            }
        ...
        public YmapCarGen NewCarGen(YmapCarGen copy = null, bool copyPosition = false, bool selectNew = true)
        {
            ...
            {
                var uymap = CurrentYmapFile; var ucg = cg;
                RegisterUndoItem("Create car generator",
                    () => RunLocked(() => uymap.AddCarGen(ucg)),
                    () => RunLocked(() => uymap.RemoveCarGen(ucg)));
            }
        ...
        public bool DeleteCarGen()
        {
            ...
            {
                var uymap = CurrentYmapFile; var ucg = delgen;
                RegisterUndoItem("Delete car generator",
                    () => RunLocked(() => uymap.RemoveCarGen(ucg)),
                    () => RunLocked(() => uymap.AddCarGen(ucg)));
            }
        ...
        public YmapLODLight NewLodLight(YmapLODLight copy = null, bool copyPosition = false, bool selectNew = true)
        {
            ...
            {
                var uymap = CurrentYmapFile; var ull = yll;
                RegisterUndoItem("Create LOD light",
                    () => RunLocked(() => { uymap.AddLodLight(ull); WorldForm?.UpdateLodLightGraphics(ull); }),
                    () => RunLocked(() => { uymap.RemoveLodLight(ull); WorldForm?.UpdateLodLightGraphics(ull); }));
            }
        ...
        public bool DeleteLodLight()
        {
            ...
            {
                var uymap = CurrentYmapFile; var ull = delyll;
                RegisterUndoItem("Delete LOD light",
                    () => RunLocked(() => { uymap.RemoveLodLight(ull); WorldForm?.UpdateLodLightGraphics(ull); }),
                    () => RunLocked(() => { uymap.AddLodLight(ull); WorldForm?.UpdateLodLightGraphics(ull); }));
            }
        ...
        public YmapBoxOccluder NewBoxOccluder(YmapBoxOccluder copy = null, bool copyPosition = false, bool selectNew = true)
        {
            ...
            {
                var uymap = CurrentYmapFile; var ubo = bo;
                RegisterUndoItem("Create box occluder",
                    () => RunLocked(() => { uymap.AddBoxOccluder(ubo); WorldForm?.UpdateBoxOccluderGraphics(ubo); }),
                    () => RunLocked(() => { uymap.RemoveBoxOccluder(ubo); WorldForm?.UpdateBoxOccluderGraphics(ubo); }));
            }
        ...
        public bool DeleteBoxOccluder()
        {
            ...
            {
                var uymap = CurrentYmapFile; var ubo = delbox;
                RegisterUndoItem("Delete box occluder",
                    () => RunLocked(() => { uymap.RemoveBoxOccluder(ubo); WorldForm?.UpdateBoxOccluderGraphics(ubo); }),
                    () => RunLocked(() => { uymap.AddBoxOccluder(ubo); WorldForm?.UpdateBoxOccluderGraphics(ubo); }));
            }
        ...
        public YmapOccludeModel NewOccludeModel(YmapOccludeModel copy = null, bool copyPosition = false, bool selectNew = true)
        {
            ...
            {
                var uymap = CurrentYmapFile; var uom = om;
                RegisterUndoItem("Create occlude model",
                    () => RunLocked(() => { uymap.AddOccludeModel(uom); WorldForm?.UpdateOccludeModelGraphics(uom); }),
                    () => RunLocked(() => { uymap.RemoveOccludeModel(uom); WorldForm?.UpdateOccludeModelGraphics(uom); }));
            }
        ...
        public bool DeleteOccludeModel()
        {
            ...
            {
                var uymap = CurrentYmapFile; var uom = delmodel;
                RegisterUndoItem("Delete occlude model",
                    () => RunLocked(() => { uymap.RemoveOccludeModel(uom); WorldForm?.UpdateOccludeModelGraphics(uom); }),
                    () => RunLocked(() => { uymap.AddOccludeModel(uom); WorldForm?.UpdateOccludeModelGraphics(uom); }));
            }
        ...
        public YmapOccludeModelTriangle NewOccludeModelTriangle(YmapOccludeModelTriangle copy = null, bool copyPosition = false, bool selectNew = true)
        {
            ...
            {
                var uymap = CurrentYmapFile; var uot = ot;
                RegisterUndoItem("Create occlude triangle",
                    () => RunLocked(() => { uymap.AddOccludeModelTriangle(uot); WorldForm?.UpdateOccludeModelGraphics(uot.Model); }),
                    () => RunLocked(() => { uymap.RemoveOccludeModelTriangle(uot); WorldForm?.UpdateOccludeModelGraphics(uot.Model); }));
            }
        ...
        public bool DeleteOccludeModelTriangle()
        {
            ...
            {
                var uymap = CurrentYmapFile; var uot = deltri;
                RegisterUndoItem("Delete occlude triangle",
                    () => RunLocked(() => { uymap.RemoveOccludeModelTriangle(uot); WorldForm?.UpdateOccludeModelGraphics(uot.Model); }),
                    () => RunLocked(() => { uymap.AddOccludeModelTriangle(uot); WorldForm?.UpdateOccludeModelGraphics(uot.Model); }));
            }
        ...
        public bool DeleteMloEntity()
        {
            ...
            {
                MessageBox.Show("This entity's numChildren is not 0 - deleting entities with children is not currently supported by EvelentWalker.");
                return true;
            }
        ...
        public AudioPlacement NewAudioAmbientZone(AudioPlacement copy = null, bool copyPosition = false, bool selectNew = true)
        {
            ...
            {
                var arel = CurrentAudioFile; var aobj = zone;
                RegisterUndoItem("Create audio zone",
                    () => RunLocked(() => { arel.AddRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }),
                    () => RunLocked(() => { arel.RemoveRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }));
            }
        ...
        public bool DeleteAudioAmbientZone()
        {
            ...
            {
                var arel = delrel; var aobj = delzone.AmbientZone;
                RegisterUndoItem("Delete audio zone",
                    () => RunLocked(() => { arel.RemoveRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }),
                    () => RunLocked(() => { arel.AddRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }));
            }
        ...
        public AudioPlacement NewAudioAmbientRule(AudioPlacement copy = null, bool copyPosition = false, bool selectNew = true)
        {
            ...
            {
                var arel = CurrentAudioFile; var aobj = rule;
                RegisterUndoItem("Create audio rule",
                    () => RunLocked(() => { arel.AddRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }),
                    () => RunLocked(() => { arel.RemoveRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }));
            }
        ...
        public bool DeleteAudioAmbientRule()
        {
            ...
            {
                var arel = delrel; var aobj = delem.AmbientRule;
                RegisterUndoItem("Delete audio rule",
                    () => RunLocked(() => { arel.RemoveRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }),
                    () => RunLocked(() => { arel.AddRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }));
            }
        ...
        public AudioPlacement NewAudioStaticEmitter(AudioPlacement copy = null, bool copyPosition = false, bool selectNew = true)
        {
            ...
            {
                var arel = CurrentAudioFile; var aobj = emitter;
                RegisterUndoItem("Create static emitter",
                    () => RunLocked(() => { arel.AddRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }),
                    () => RunLocked(() => { arel.RemoveRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }));
            }
        ...
        public bool DeleteAudioStaticEmitter()
        {
            ...
            {
                var arel = delrel; var aobj = delem.StaticEmitter;
                RegisterUndoItem("Delete static emitter",
                    () => RunLocked(() => { arel.RemoveRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }),
                    () => RunLocked(() => { arel.AddRelData(aobj); WorldForm?.UpdateAudioPlacementGraphics(arel); }));
            }
        ...
        public void NewAudioAmbientZoneList()
        {
            ...
            {
                var arel = CurrentAudioFile; var aobj = zonelist;
                RegisterUndoItem("Create audio zone list",
                    () => RunLocked(() => arel.AddRelData(aobj)),
                    () => RunLocked(() => arel.RemoveRelData(aobj)));
            }
        ...
        public bool DeleteAudioAmbientZoneList()
        {
            ...
            {
                var arel = delrel; var aobj = delzl;
                RegisterUndoItem("Delete audio zone list",
                    () => RunLocked(() => arel.RemoveRelData(aobj)),
                    () => RunLocked(() => arel.AddRelData(aobj)));
            }
        ...
        public void NewAudioStaticEmitterList()
        {
            ...
            {
                var arel = CurrentAudioFile; var aobj = emlist;
                RegisterUndoItem("Create emitter list",
                    () => RunLocked(() => arel.AddRelData(aobj)),
                    () => RunLocked(() => arel.RemoveRelData(aobj)));
            }
        ...
        public bool DeleteAudioStaticEmitterList()
        {
            ...
            {
                var arel = delrel; var aobj = delel;
                RegisterUndoItem("Delete emitter list",
                    () => RunLocked(() => arel.RemoveRelData(aobj)),
                    () => RunLocked(() => arel.AddRelData(aobj)));
            }
        ...
        public void NewAudioInterior()
        {
            ...
            {
                var arel = CurrentAudioFile; var aobj = interior;
                RegisterUndoItem("Create audio interior",
                    () => RunLocked(() => arel.AddRelData(aobj)),
                    () => RunLocked(() => arel.RemoveRelData(aobj)));
            }
        ...
        public bool DeleteAudioInterior()
        {
            ...
            {
                var arel = delrel; var aobj = delel;
                RegisterUndoItem("Delete audio interior",
                    () => RunLocked(() => arel.RemoveRelData(aobj)),
                    () => RunLocked(() => arel.AddRelData(aobj)));
            }
        ...
        public void NewAudioInteriorRoom()
        {
            ...
            {
                var arel = CurrentAudioFile; var aobj = room;
                RegisterUndoItem("Create interior room",
                    () => RunLocked(() => arel.AddRelData(aobj)),
                    () => RunLocked(() => arel.RemoveRelData(aobj)));
            }
        ...
        public bool DeleteAudioInteriorRoom()
        {
            ...
            {
                var arel = delrel; var aobj = delel;
                RegisterUndoItem("Delete interior room",
                    () => RunLocked(() => arel.RemoveRelData(aobj)),
                    () => RunLocked(() => arel.AddRelData(aobj)));
            }
        ...
        //called from WorldForm undo/redo after a ymap's entity graph was restored
        public void OnEntityGraphChanged(YmapFile ymap)
        {
            try
            {
                if (InvokeRequired)
                {
                    BeginInvoke(new Action(() => { OnEntityGraphChanged(ymap); }));
                    return;
                }
                ...
                if ((ymap != null) && (WorldForm != null))
                {
                    //refresh the 3D view for the affected ymap
                    lock (WorldForm.RenderSyncRoot)
                    {
                        ymap.HasChanged = true;
        ...
        // ---------------------------------------------------------------------
        //  Generic undo/redo plumbing for create/delete operations.
        //  Each operation registers a step whose redo re-applies it and whose
        //  undo reverses it, reusing the existing Add*/Remove* methods.
        // ---------------------------------------------------------------------
        private void RunLocked(Action a)
        {
            if (a == null) return;
            if (WorldForm != null) { lock (WorldForm.RenderSyncRoot) { a(); } }
            else a();
        }

        private void UndoRedoRefresh()
        {
            if (InvokeRequired) { try { Invoke(new Action(UndoRedoRefresh)); } catch { } return; }
            try { LoadProjectTree(); } catch { }
        }

        //redo = re-apply the operation, undo = reverse it. Both refresh the tree afterwards.
        private void RegisterUndoItem(string desc, Action redo, Action undo)
        {
            if (WorldForm == null) return;
            WorldForm.RegisterUndoStep(new RelayUndoStep(desc,
                () => { undo?.Invoke(); UndoRedoRefresh(); },
                () => { redo?.Invoke(); UndoRedoRefresh(); }));
        }

        //enables/disables the Undo/Redo toolbar buttons + Edit menu items (called from WorldForm)
        public void UpdateUndoRedoButtons(bool canUndo, bool canRedo)
        {
            try
            {
                if (InvokeRequired)
                {
                    BeginInvoke(new Action(() => UpdateUndoRedoButtons(canUndo, canRedo)));
                    return;
                }
                ToolbarUndoButton.Enabled = canUndo;
                ToolbarRedoButton.Enabled = canRedo;
                EditUndoMenu.Enabled = canUndo;
                EditRedoMenu.Enabled = canRedo;
            }
            catch { }
        }
        ...
        {
            WorldForm?.PerformUndo();
        }
        private void ToolbarRedoButton_Click(object sender, EventArgs e)
        {
            WorldForm?.PerformRedo();
        }
        private void EditUndoMenu_Click(object sender, EventArgs e)
        {
            WorldForm?.PerformUndo();
        }
        private void EditRedoMenu_Click(object sender, EventArgs e)
        {
            WorldForm?.PerformRedo();
        }
        ...
        private void UpdateFormTitleText()
        {
            if (CurrentProjectFile == null)
            {
                Text = "Project - EvelentWalker";
            }
            else
            {
                Text = CurrentProjectFile.Name + " - EvelentWalker";
            }
        }
        ...
        private void ProjectForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (CurrentProjectFile?.HasChanged ?? false)
            {
                ...
                {
                    msg = "Are you sure you want to quit EvelentWalker?";
                    tit = "Confirm quit";
                }
