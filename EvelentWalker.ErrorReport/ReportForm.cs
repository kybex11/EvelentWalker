...
namespace EvelentWalker.ErrorReport
{
        ...
        private void ReportForm_Load(object sender, EventArgs e)
        {
            ...
            for (int i = appLog.Entries.Count - 1; i >= 0; i--)
            {
                ...
                if (entry.EntryType == EventLogEntryType.Error && entry.Source == ".NET Runtime")
                {
                    ...
                    if (lines.Length > 0 &&
                        (lines[0].Contains("EvelentWalker.exe") ||
                         lines[0].Contains("EvelentWalker RPF Explorer.exe") ||
                         lines[0].Contains("EvelentWalker Ped Viewer.exe") ||
                         lines[0].Contains("EvelentWalker Vehicle Viewer.exe")))
                    {
                        found = true;
            ...
            if (!found)
            {
                ...
                MessageBox.Show("Unable to find the last EvelentWalker error in the Event Log.");
            }
        }
