...
using EvelentWalker.Core.Utils;
using EvelentWalker.World;

namespace EvelentWalker.GameFiles
{
...
//#endif
        ...
        public byte[] Save()
        {
            ...
            block.exportedBy = mb.AddStringPtr("EvelentWalker");
        ...
        public bool RemoveEntity(YmapEntityDef ent)
        {
            ...
            var newAllEntities = new List<YmapEntityDef>();
            bool removed = false;
            for (int i = 0; i < AllEntities.Length; i++)
            {
                var oent = AllEntities[i];
                if (oent == ent) { removed = true; continue; } //match by reference, so a stale Index can't block removal
                oent.Index = newAllEntities.Count;
                newAllEntities.Add(oent);
            }

            if (!removed) return false; //entity isn't part of this ymap

            var newRootEntities = new List<YmapEntityDef>();
            if (RootEntities != null)
            {
                for (int i = 0; i < RootEntities.Length; i++)
                {
                    var oent = RootEntities[i];
                    if (oent != ent) newRootEntities.Add(oent); //a child entity simply isn't in here - that's fine
                }
            }

            //keep the LOD parent consistent: drop this entity from its parent's
            //child list and decrement numChildren, so the game doesn't try to
            //stream a child that no longer exists.
            ...
            FixEntityParentIndices(); //repair parentIndex links after re-indexing so LOD links survive a save
        ...
        /// <summary>
        /// Remove a whole set of entities at once (e.g. a complete LOD group: a
        /// parent and all of its children). After removal the remaining entities
        /// are re-indexed and their parentIndex values are repaired from the
        /// (maintained) Parent references, so the saved ymap keeps valid LOD
        /// linkage and won't corrupt the game.
        /// </summary>
        public bool RemoveEntities(IList<YmapEntityDef> ents)
        {
            if ((ents == null) || (ents.Count == 0)) return false;
            if (AllEntities == null) return false;

            var remset = new HashSet<YmapEntityDef>(ents);

            var newAllEntities = new List<YmapEntityDef>();
            for (int i = 0; i < AllEntities.Length; i++)
            {
                var oent = AllEntities[i];
                if (remset.Contains(oent)) continue;
                oent.Index = newAllEntities.Count;
                newAllEntities.Add(oent);
            }
            ...
            if (RootEntities != null)
            {
                ...
                {
                    var oent = RootEntities[i];
                    if (!remset.Contains(oent)) newRootEntities.Add(oent);
                }
            }

            bool removedAny = newAllEntities.Count != AllEntities.Length;

            LodManagerOldEntities = AllEntities;
            AllEntities = newAllEntities.ToArray();
            RootEntities = newRootEntities.ToArray();

            FixEntityParentIndices();
        ...
        /// <summary>
        /// Rewrites every entity's CEntityDef.parentIndex from its (maintained)
        /// Parent reference and the current AllEntities ordering. Entities that
        /// are roots (no in-ymap parent) or whose LOD parent lives in another
        /// ymap (LodInParentYmap) are left untouched, since their parentIndex
        /// does not refer to this ymap's entity array.
        /// </summary>
        public void FixEntityParentIndices()
        {
            if (AllEntities == null) return;
            var present = new HashSet<YmapEntityDef>(AllEntities);
            for (int i = 0; i < AllEntities.Length; i++)
            {
                var ent = AllEntities[i];
                if (ent == null) continue;
                if (ent.LodInParentYmap) continue; //parentIndex refers to another ymap - don't touch
                var par = ent.Parent;
                if (par == null) continue; //root - leave parentIndex as-is
                if (par.Ymap != this) continue; //cross-ymap LOD parent - leave as-is

                if (present.Contains(par))
                {
                    ent._CEntityDef.parentIndex = par.Index; //repoint to parent's new index
                }
                else
                {
                    //parent was removed - turn this into a root so no dangling LOD link is saved
                    ent.Parent = null;
                    ent._CEntityDef.parentIndex = -1;
                }
            }
        }

        /// <summary>
        /// Detaches an entity from its (in-ymap) LOD parent: removes it from the
        /// parent's Children array and decrements the parent's numChildren so the
        /// saved ymap stays consistent and won't reference a missing child.
        /// </summary>
        private void DetachFromParent(YmapEntityDef ent)
        {
            var par = ent?.Parent;
            if (par == null) return;
            if (par.Ymap != this) return; //cross-ymap parent - can't safely touch

            if ((par.Children != null) && (par.Children.Length > 0))
            {
                var list = new List<YmapEntityDef>(par.Children.Length);
                foreach (var c in par.Children) { if (c != ent) list.Add(c); }
                par.Children = list.ToArray();
                par.ChildrenMerged = par.Children;
            }
            if (par._CEntityDef.numChildren > 0)
            {
                par._CEntityDef.numChildren--;
            }
            ent.Parent = null;
        }

        /// <summary>
        /// Captures a full snapshot of the entity graph (the AllEntities/
        /// RootEntities arrays plus each entity's CEntityDef, Parent and Children)
        /// so it can be restored later. Used to make entity add/delete undoable.
        /// </summary>
        public YmapEntityState CaptureEntityState()
        {
            var st = new YmapEntityState();
            var all = AllEntities ?? new YmapEntityDef[0];
            st.AllEntities = (YmapEntityDef[])all.Clone();
            st.RootEntities = (RootEntities != null) ? (YmapEntityDef[])RootEntities.Clone() : new YmapEntityDef[0];
            int n = all.Length;
            st.Defs = new CEntityDef[n];
            st.Parents = new YmapEntityDef[n];
            st.Children = new YmapEntityDef[n][];
            st.ChildrenMerged = new YmapEntityDef[n][];
            st.Indices = new int[n];
            for (int i = 0; i < n; i++)
            {
                var e = all[i];
                st.Defs[i] = e._CEntityDef;
                st.Parents[i] = e.Parent;
                st.Children[i] = e.Children;
                st.ChildrenMerged[i] = e.ChildrenMerged;
                st.Indices[i] = e.Index;
            }
            return st;
        }

        /// <summary>
        /// Restores an entity-graph snapshot previously taken with
        /// <see cref="CaptureEntityState"/>.
        /// </summary>
        public void RestoreEntityState(YmapEntityState st)
        {
            if (st == null) return;

            if (st.AllEntities != null)
            {
                for (int i = 0; i < st.AllEntities.Length; i++)
                {
                    var e = st.AllEntities[i];
                    if (e == null) continue;
                    e.Ymap = this;
                    e._CEntityDef = st.Defs[i];
                    e.Parent = st.Parents[i];
                    e.Children = st.Children[i];
                    e.ChildrenMerged = st.ChildrenMerged[i];
                    e.Index = st.Indices[i];
                }
            }

            LodManagerOldEntities = AllEntities;
            AllEntities = (st.AllEntities != null) ? (YmapEntityDef[])st.AllEntities.Clone() : null;
            RootEntities = (st.RootEntities != null) ? (YmapEntityDef[])st.RootEntities.Clone() : null;
    ...
    /// <summary>
    /// Immutable snapshot of a ymap's entity graph used for undo/redo of entity
    /// add/delete operations. Captured via <see cref="YmapFile.CaptureEntityState"/>.
    /// </summary>
    public class YmapEntityState
    {
        public YmapEntityDef[] AllEntities;
        public YmapEntityDef[] RootEntities;
        public CEntityDef[] Defs;
        public YmapEntityDef[] Parents;
        public YmapEntityDef[][] Children;
